...
> 🔧 **未実装の対応方針あり**: 実機取込で出た指摘(メモ100文字超え / 品目「ほかN点」/
> カテゴリ内訳の2択 / 出力行の選択)への対応方針は
> [`PLAN_ITEM_MEMO_SELECTION.md`](PLAN_ITEM_MEMO_SELECTION.md) にまとめてある。
> これから実装するセッションは**まずそれを読む**こと。
...
- **品目**: `shortenName()` で `【まとめ買い】` のような宣伝ブロックを落とし、24 文字で切る。
  同名は `×N`、複数種類は `先頭 ほかN点`(`itemLabel()`)。
- **メモ**: `buildMemo()` が**全商品名 + 注記 + 注文 ID** を入れる。
