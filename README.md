...
> (レシート記帳)。CSV の行を増やしても Zaim 上の記録数は増えません(実データでは 39行 → 30記録)。
> **これは許容する方針**です ― 合計は正しく、各注文の金額と注文IDは記録の中の品目として残るため。
> 経緯は `docs/IMPLEMENTATION.md` §11-6。

詳細・根拠・検証値は `docs/IMPLEMENTATION.md` を参照。

## 実機 Zaim での取込テスト(2026-09-12・成功)

ダミー6行を Zaim の「アップロードのテスト」に通し、**メモ・品目・内訳・支払元・金額が
すべて想定どおり**に入ることを確認済み(品目は52文字まで赤字なし)。詳細は
`docs/IMPLEMENTATION.md` §11-10。

> 取込時に **「カテゴリと内訳の追加はプレミアム機能」**と出て全件弾かれたら、
> まず**ログインしているアカウント**を疑うこと(別アカウントのセッションが残っていた実例あり)。
> 次に、CSV の内訳・カテゴリが Zaim に実在するかを確認する
> (無料プランでは CSV から新しい内訳を作れない)。
